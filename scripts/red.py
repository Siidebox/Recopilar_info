...
def command_exists(command):
    if platform.system() == "Windows":
        ...
        result = subprocess.run(f"where {command}", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    else:
...
def execute_nmap_scan(targets, ports):
    ...
    try:
            ...
            # Detecta los puertos y servicios
                ...
                if current_host:
                    ...
                    port_info = {
                        ...
                        "Versión": " ".join(port_data[3:]) if len(port_data) > 3 else "Desconocida"
                    }
        ...
        print(f"Error al ejecutar Nmap: {str(e)}. Asegúrate de que Nmap esté instalado y que tengas permisos suficientes para ejecutarlo.")
...
def main():
    ...
    # Nota sobre permisos
    print("Nota: Es posible que necesites permisos administrativos para ejecutar el escaneo de puertos.")
    ...
    for host, info in scan_results.items():
        ...
        for port_info in info["Puertos"]:
            print(f"  Puerto {port_info['Puerto']}/{port_info['Protocolo']} - {port_info['Estado']}")
            print(f"    Servicio: {port_info['Servicio']} - Versión: {port_info['Versión']}")
